...
from .forms import UserForm, UserUpdateForm, FileForm
from django.urls import reverse,reverse_lazy
from django.shortcuts import get_object_or_404

from App.models import FileModel, UserProfile
...
class UpdateUserPage(LoginRequiredMixin,UpdateView):
    template_name = 'users/user_form.html'
    context_object_name = 'form'
    model = User
...
class SelectPredFileView(LoginRequiredMixin,TemplateView):
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        myfiles = FileModel.objects.filter(userprofile=user_profile)
        context['filename'] = myfiles
        return context


class SelectFileDelView(LoginRequiredMixin,TemplateView):
    """
    This view is used to select a file from the list of files in the server.
    After the selection, it will send the file to the server.
    ...
    def get_context_data(self, **kwargs):
        ...
        for value in myfiles:
            primary_key_list.append(value.pk)
        file_and_pk = zip(myfiles, primary_key_list)
        context['filename'] = file_and_pk
        return context


class FileDeleteView(LoginRequiredMixin,views.APIView):
    """
    ...
    def post(self, request):
        ...
        delete_action = get_object_or_404(FileModel, pk=identifier).delete()
        try:
...
class FileView(LoginRequiredMixin, views.APIView):
    """
...
class Predict(LoginRequiredMixin, views.APIView):
    """
